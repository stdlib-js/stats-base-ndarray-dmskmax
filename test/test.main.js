...
var dmskmax = require( './../lib/main.js' );
...
tape( 'the function calculates the maximum value of a one-dimensional ndarray according to a mask', function test( t ) {
	...
	v = dmskmax( [ vector( 'float64', x, 6, 1, 0 ), vector( 'uint8', mask, 6, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = new Float64Array( [ -4.0, -5.0 ] );
	mask = new Uint8Array( [ 0, 1 ] );
	v = dmskmax( [ vector( 'float64', x, 2, 1, 0 ), vector( 'uint8', mask, 2, 1, 0 ) ] );
	t.strictEqual( v, -4.0, 'returns expected value' );

	x = new Float64Array( [ -0.0, 0.0, -0.0 ] );
	mask = new Uint8Array( [ 1, 0, 0 ] );
	v = dmskmax( [ vector( 'float64', x, 3, 1, 0 ), vector( 'uint8', mask, 3, 1, 0 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having non-unit strides', function test( t ) {
	...
	v = dmskmax( [ vector( 'float64', x, 4, 2, 0 ), vector( 'uint8', mask, 4, 2, 0 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having negative strides', function test( t ) {
	...
	v = dmskmax( [ vector( 'float64', x, 4, -2, 6 ), vector( 'uint8', mask, 4, -2, 6 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having non-zero offsets', function test( t ) {
	...
	v = dmskmax( [ vector( 'float64', x, 4, 2, 1 ), vector( 'uint8', mask, 4, 2, 1 ) ] );
	t.strictEqual( v, 4.0, 'returns expected value' );
